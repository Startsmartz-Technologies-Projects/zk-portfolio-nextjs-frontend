...
export function StoryList() {
  ...
  const columns: ColumnDef<StoryListItem>[] = [
  ...
  async function fetchPage(query: Record<string, string | undefined>): Promise<ListResult<StoryListItem>> {
    return (await listStoriesAction({
      page: query.page,
    ...
    })) as ListResult<StoryListItem>;
  }

  return (
    <FilteredList<StoryListItem>
      fetchPage={fetchPage}
      basePath="/admin/news"
      noun="story"
      nounPlural="stories"
      ...
      columns={columns}
      filters={[
        { kind: "taxonomy", key: "category", vocabularySlug: "news-category", placeholder: "All categories" },
        {
          kind: "enum",
          key: "contentStatus",
          label: "Status",
      ...
      defaultSort="latest"
      bulk={async (ids, action) => bulkStoriesAction({ ids, action })}
      rowActions={(row, reload) => [
        {
          ...
          onSelect: async () => {
            try {
              const copy = await duplicateStoryAction(row.id);
              toast({ variant: "success", title: "Duplicated — opening the copy." });
              router.push(`/admin/news/${copy.id}`);
            } catch {
              toast({ variant: "destructive", title: "Couldn't duplicate the story." });
            }
          },
        },
        {
          ...
          onSelect: async () => {
            try {
              const { preview_url } = await previewStoryAction(row.id);
        ...
        },
        {
          ...
          onSelect: async () => {
            ...
            if (!ok) return;
            try {
              await deleteStoryAction(row.id);
              toast({ variant: "success", title: "Deleted." });
              reload();
            } catch {
              toast({ variant: "destructive", title: "Couldn't delete the story." });
            }
          },
        },
      ]}
